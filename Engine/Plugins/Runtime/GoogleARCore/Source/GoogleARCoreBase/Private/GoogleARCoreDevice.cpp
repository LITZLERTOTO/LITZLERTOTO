...
FGoogleARCoreDevice::FGoogleARCoreDevice()
	...
	, CurrentSessionStatus(EGoogleARCoreSessionStatus::NotStarted)
{
}

void FGoogleARCoreDevice::OnModuleLoaded()
{
	ProjectARCoreConfig = GetDefault<UGoogleARCoreEditorSettings>()->DefaultSessionConfig;
	...
	if (ARCoreSession->GetSessionCreateStatus() == EGoogleARCoreAPIStatus::AR_SUCCESS)
	{
		...
		const bool bStartInAR = GetDefault<UGeneralProjectSettings>()->bStartInAR;
		if (bStartInAR)
		{
			StartARCoreSessionRequest(ProjectARCoreConfig);
		}
	}
}
...
EGoogleARCoreSessionStatus FGoogleARCoreDevice::GetSessionStatue()
{
...
void FGoogleARCoreDevice::StartARCoreSessionRequest()
{
	StartARCoreSessionRequest(ProjectARCoreConfig);
}
...
void FGoogleARCoreDevice::StartARCoreSessionRequest(const FGoogleARCoreSessionConfig& SessionConfig)
{
	...
	if (ARCoreSession->GetSessionCreateStatus() != EGoogleARCoreAPIStatus::AR_SUCCESS)
	{
		CurrentSessionStatus = EGoogleARCoreSessionStatus::ARCoreNotSupported;
		bStartSessionRequested = false;
		return;
	}
	...
	if (CurrentSessionStatus == EGoogleARCoreSessionStatus::FatalError)
	{
...
void FGoogleARCoreDevice::OnWorldTickStart(ELevelTick TickType, float DeltaTime)
{
	...
	if (!bIsARCoreSessionRunning && bStartSessionRequested)
	{
		...
		{
			CurrentSessionStatus = EGoogleARCoreSessionStatus::PermissionNotGranted;
			bStartSessionRequested = false;
		}
		else
		{
	...
	if (bIsARCoreSessionRunning)
	{
		...
		if (Status == EGoogleARCoreAPIStatus::AR_ERROR_FATAL)
		{
			ARCoreSession->Pause();
			bIsARCoreSessionRunning = false;
			CurrentSessionStatus = EGoogleARCoreSessionStatus::FatalError;
		}
		else
		{
...
void FGoogleARCoreDevice::StartSession(const FGoogleARCoreSessionConfig& ConfigurationData)
{
	...
	if (Status != EGoogleARCoreAPIStatus::AR_SUCCESS)
	{
		...
		CurrentSessionStatus = EGoogleARCoreSessionStatus::UnSupportedConfiguration;
		return;
	}
...
void FGoogleARCoreDevice::ARLineTrace(const FVector2D& ScreenPosition, TSet<EGoogleARCoreLineTraceChannel> ARLineTraceChannels, TArray<FGoogleARCoreTraceResult>& OutHitResults)
{
	...
	for (FGoogleARCoreTraceResult HitResult : AllHitResults)
	{
		...
		if (TrackableType == UGoogleARCoreTrackablePoint::StaticClass() && ARLineTraceChannels.Contains(EGoogleARCoreLineTraceChannel::FeaturePoint))
		{
			OutHitResults.Add(HitResult);
		}
		else if (TrackableType == UGoogleARCorePlane::StaticClass())
		{
			if (ARLineTraceChannels.Contains(EGoogleARCoreLineTraceChannel::InfinitePlane))
			{
				OutHitResults.Add(HitResult);
			}
			else
			{
				...
				if (ARLineTraceChannels.Contains(EGoogleARCoreLineTraceChannel::PlaneUsingExtent) && Plane->IsPointInExtents(HitPoint))
				{
					OutHitResults.Add(HitResult);
				}
				else if (ARLineTraceChannels.Contains(EGoogleARCoreLineTraceChannel::PlaneUsingBoundaryPolygon) && Plane->IsPointInPolygon(HitPoint))
				{
					OutHitResults.Add(HitResult);
				}
			}
