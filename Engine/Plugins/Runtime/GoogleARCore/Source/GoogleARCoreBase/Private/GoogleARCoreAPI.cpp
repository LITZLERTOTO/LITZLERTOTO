...
FGoogleARCoreSession::~FGoogleARCoreSession()
{
	for (UGoogleARCoreAnchor* Anchor : UObjectManager->AllAnchors)
	{
		Anchor->TrackingState = EGoogleARCoreTrackingState::StoppedTracking;
	}
...
EGoogleARCoreAPIStatus FGoogleARCoreSession::Pause()
{
	...
	for (UGoogleARCoreAnchor* Anchor : UObjectManager->AllAnchors)
	{
		Anchor->TrackingState = EGoogleARCoreTrackingState::NotTracking;
	}
#endif
	return PauseStatue;
}
...
{
	EGoogleARCoreTrackingState TrackingState = EGoogleARCoreTrackingState::StoppedTracking;
...
EGoogleARCoreTrackablePointOrientationMode UGoogleARCoreTrackablePoint::GetPointOrientationMode()
{
	EGoogleARCoreTrackablePointOrientationMode PointOrientationMode = EGoogleARCoreTrackablePointOrientationMode::Identity;
	if (CheckIsSessionValid("ARCoreTrackablePoint", Session))
	{
#if PLATFORM_ANDROID
		ArSession* SessionHandle = Session.Pin()->GetHandle();
		ArPointOrientationMode OrientationMode = AR_POINT_ORIENTATION_INITIALIZED_TO_IDENTITY;
		ArPoint_getOrientationMode(SessionHandle, GetPointHandle(), &OrientationMode);

		PointOrientationMode = static_cast<EGoogleARCoreTrackablePointOrientationMode>(OrientationMode);
#endif
	}
	return PointOrientationMode;
}
